    ...
    * Su código debe crear la carpeta `docs` si no existe.
    ...
    def plots(df, output_directory):
        ...
        def create_visual_for_shipping_per_warehouse(df, output_directory):
            ...
            df = df.copy()
            counts = df.Warehouse_block.value_counts()  #devuelve el conteo
            plt.figure()
            counts.plot.bar(
        ...
        def create_visual_for_mode_of_shipment(df, output_directory):
            ...
            counts.plot.pie(
                ...
                wedgeprops=dict(width=0.35),   #ancho 35% 
                ylabel="",
        ...
        def create_visual_for_average_customer_rating(df, output_directory):
            ...
            df = df.copy()
            df = (
                df[["Mode_of_Shipment", "Customer_rating"]]  #doble corche para seleccionar y crear al df
                .groupby("Mode_of_Shipment")                 #mediante el groupby se calculan las estadíticas (describe)
                .describe()
            )
            df.columns = df.columns.droplevel()              #Eliminar el nombre encima de la columna (droplevel)
            df = df[["mean", "min", "max"]]
        ...
        # Ejecuta las visuales donde se generan las figuras
        plt.tight_layout()
